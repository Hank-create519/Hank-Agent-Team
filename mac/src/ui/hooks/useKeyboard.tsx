  ...
  const showCommandPalette = useCallback(() => {
    ...
    setCommandPalette(
      ...
      >
        ...
        {[
          { label: '总览', keys: '⌘1', action: () => onNavigatePage(0) },
          { label: '流水线', keys: '⌘2', action: () => onNavigatePage(1) },
          { label: '审查详情', keys: '⌘3', action: () => onNavigatePage(2) },
          { label: '监控', keys: '⌘4', action: () => onNavigatePage(3) },
          { label: '团队', keys: '⌘5', action: () => onNavigatePage(4) },
          { label: '切换右侧面板', keys: '⌘K', action: onToggleRightPanel },
          { label: '执行审查', keys: '⌘Enter', action: onSubmitInput },
        ].map((cmd) => (
          <div
            key={cmd.label}
            style={{
