...
const App: React.FC = () => {
  ...
  // ===== Windows 自定义标题栏状态 =====
  const isWin =
    typeof window !== 'undefined' && (window as any).electronAPI?.platform === 'win32';
  const [isMaximized, setIsMaximized] = useState(false);
  useEffect(() => {
    const api = (window as any).electronAPI;
    if (api?.platform === 'win32') {
      api.isMaximized().then(setIsMaximized).catch(() => {});
      api.onMaximizedChange((v: boolean) => setIsMaximized(v));
    }
  }, []);
  const ctrl = (action: string) => (window as any).electronAPI?.windowControl(action);
  const winCtrlBtnStyle: React.CSSProperties = {
    width: 44,
    height: 38,
    display: 'flex',
    alignItems: 'center',
    ...
    <div style={{ display: 'flex', height: '100vh', width: '100vw', overflow: 'hidden' }}>
      ...
      >
        ...
        >
          ...
          {/* Windows 自定义标题栏控制按钮 */}
          {isWin && (
            <div style={{ display: 'flex', marginLeft: 8, ...dragRegion('no-drag') }}>
              <button
                onClick={() => ctrl('minimize')}
                title="最小化"
                style={winCtrlBtnStyle}
